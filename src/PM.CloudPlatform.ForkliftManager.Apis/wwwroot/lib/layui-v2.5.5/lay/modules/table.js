...
layui.define(["laytpl", "laypage", "layer", "form", "util"], function(e) {
	...
	j.prototype.config = {
			...
			if (i.startTime = (new Date).getTime(), a.url) {
				...
				if(a.where.isPage == true){ //分页功能
					// 处理where参数
					var tabelWhereDataP = JSON.parse(a.where.data).pagination;
					var tabelWhereDataQ = JSON.parse(a.where.data).queryJson;
					tabelWhereDataP.rows = r[l.limitName];
					tabelWhereDataP.page = r[l.pageName];
					var tabelWhereData2={};
					tabelWhereData2.pagination = tabelWhereDataP;
					tabelWhereData2.queryJson = tabelWhereDataQ;
					a.where.data = JSON.stringify(tabelWhereData2)
					// 处理where参数结束
				}
				...
				i.tipsIndex = l.tips(['<div class="layui-table-tips-main" style="margin-top: -' + (d.height() + 16) + "px;" +
					function() {
						return "sm" === a.size ? "padding: 4px 15px; font-size: 12px;" : "lg" === a.size ? "padding: 14px 15px;" :
							""
					}() + '">', d.html().replace(/\n/g,"<br/>"), "</div>", '<i class="layui-icon layui-table-tips-c layui-icon-close"></i>'
				].join(""), d[0], {
					tips: [3, ""],
					time: -1,
					anim: -1,
